...
```{r probgrid, fig.height=5}
result <- c()

n_managers <- length(unique(all$manager))

for(place in (1:n_managers)) {
  result <- rbind(
    result,
    season_model %>%
      group_by(itr) %>%
      slice(which(
        final_score == sort(final_score, decreasing = T)[place]
      )) %>%
      group_by(manager) %>%
      summarise(
        n = n(),
  ...
  if(any(
    !(unique(all$manager) %in% result[result$place==place,]$manager)
  )){
    result <- rbind(result,
data.frame(
  manager = unique(all$manager)[!(unique(all$manager) %in% result[result$place==place,]$manager)],
  n = 0,
  mean = NA,
  pct = 0,
  label = "0%",
...
result$manager <- factor(result$manager, 
                         levels = result$manager[order(-result$pct[result$place == 1])])

orindals <- c("1st", "2nd", "3rd", paste0(4:20, "th"))

ggplot(result, aes(place, 1, colour = pct, fill = pct, label = label)) +
  ...
  xlab("Final Position") + 
  ylab("") + theme(axis.text.y  = element_blank(), legend.position = "none",
                   axis.ticks.y = element_blank()) +
  scale_x_continuous(expand = c(0,1),breaks = (1:(n_managers)), labels = orindals[(1:(n_managers))])
...
season_model %>% 
 ...
 # mutate(rank = rank(-final_score)) %>%
  ggplot(aes(manager, final_score, fill = manager)) + 
  geom_violin(alpha= 0.5, scale = "width") +
  geom_boxplot(width = 0.1) +
  coord_flip()

season_model %>% 
  ggplot(aes(final_score, fill = manager)) + 
  geom_density(adjust =3, alpha =0.5) + ylab("probability)")
