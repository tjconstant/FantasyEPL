...
```{r linreg}
ggplot(all, aes(event, total_points, colour = name, group = name))  + geom_point() + geom_smooth(method = "lm", fullrange = TRUE, se = F) + xlim(NA,38) + xlab("Gameweek") + ylab("Total Points")#+ xlim(as.Date("2017-08-11"), as.Date("2018-05-13")) 
```
...
```{r value}
ggplot(all, aes(gw_date, value.1/10, colour = name, group = name))  + geom_point() + geom_smooth() + xlab("Gameweek Date") + ylab("Team Value (£ million)")
```
