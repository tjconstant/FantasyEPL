...
form.hooper<-table.hooper$OP[(length(table.hooper$OP)-4):length(table.hooper$OP)]
form.sd.hooper<-sd(table.hooper$GP[(length(table.hooper$GP)-4):length(table.hooper$GP)])
form.fit.hooper<-lm(form.hooper ~ x.form)

#source("main.R")

for(plotRun in 1:2){
  
  if(plotRun == 2){ cairo_pdf(filename=paste("plots/currentform-extrapgraphGW",max(x),".pdf",sep=""),width=6,height=6)
  }
...
extrap.points<-(c(form.fit.breast$coefficients[1]+form.fit.breast$coefficients[2]*x.extrap,
                  ...
                  form.fit.hooper$coefficients[1]+form.fit.hooper$coefficients[2]*x.extrap))
...
for(plotRun in 1:2){
  ...
  if(plotRun == 2){cairo_pdf(filename=paste("withHooper/plots/current-form-ProbabilityDensityGW",max(x),".pdf",sep=""),width=6,height=4,pointsize=11)}
  
  x.norm<-seq(0,5000,,5000)
  plot(NA,NA,xlim=range(900,3000),ylim=c(0,0.005),xaxs="i",yaxs="i",
  ...
  gauss.plot(table.hooper,form.fit.hooper,col=rgb(1,0,0,alpha),border=10)
  
  
  legend(2450,0.00195+0.001,
