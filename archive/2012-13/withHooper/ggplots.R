...
rep("thales fc",length(table.breast$OP)),
           rep("hooper",length(table.breast$OP)))
...
library(ggplot2)
#p<-ggplot(data=ggmain,mapping=aes(x=week,y=cumpoints,color=names,group=names,size=transfersmade))
...
filename_string<-paste("withHooper/plots/TotalValueGW",max(x),".pdf",sep="")
